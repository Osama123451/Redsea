...
class _SwapSelectionPageState extends State<SwapSelectionPage> {
  ...
  Future<void> _loadUserProducts() async {
    ...
    try {
      // بما أن Realtime DB قد لا يكون مفهرساً، سنجلب الكل ونفلتر
      // أو نستخدم orderByChild إذا كان مفهرساً
      final snapshot = await ref.orderByChild('userId').equalTo(userId).once();
      ...
      if (snapshot.snapshot.value != null) {
        ...
        map.forEach((key, value) {
          try {
            ...
            // التأكد من أن المنتج ليس هو المنتج المستهدف (لا نقايض بمنتجنا نفسه إذا كنا نملكه، ولو أن هذا فحص زائد)
      ...
      builder: (context) => AddProductBottomSheet(
        onProductAdded: (newProduct) {
          setState(() {
            ...
            _filteredProducts = _userProducts;
          });
        },
      ),
    );
      ...
      child: ElevatedButton(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => SwapConfirmationPage(
